...
class _HomeScreenState extends State<HomeScreen> {
    ...
    if (isRunning) {
      ...
      if (statuses[Permission.location]!.isGranted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Location permission is required for protection")),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("SafeStep", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        centerTitle: true,
        actions: [
          IconButton(
  ...
  Widget _buildStatusCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _isServiceRunning ? Colors.green.shade100 : Colors.red.shade100),
      ),
      child: Column(
        children: [
          Icon(
            _isServiceRunning ? Icons.verified_user_rounded : Icons.gpp_bad_rounded,
            size: 64,
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: _isServiceRunning ? Colors.green.shade900 : Colors.red.shade900,
            ),
          ),
          ...
          Text(
            ...
            style: TextStyle(color: _isServiceRunning ? Colors.green.shade700 : Colors.red.shade700),
          ),
      ...
      children: [
        Row(
          ...
          children: [
            ...
            TextButton(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const ConfigurationPage())).then((_) => _loadSettings()),
  ...
  Widget _buildTriggerChip(IconData icon, String label, bool enabled) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: enabled ? Colors.black.withOpacity(0.05) : Colors.grey.withOpacity(0.05),
    ...
    return Drawer(
      child: Column(
        children: [
          ...
          _buildDrawerItem(Icons.contacts_outlined, "Contact Saving & Adding", () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => const ContactsSetupScreen()));
          }),
          ...
          _buildDrawerItem(Icons.settings_outlined, "General Settings", () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => const ConfigurationPage())).then((_) => _loadSettings());
          }),
          _buildDrawerItem(Icons.report_problem_outlined, "Report Complaint", () {
            Navigator.pop(context);
            Navigator.push(context, MaterialPageRoute(builder: (context) => const ComplaintsPage()));
          }),
          const Spacer(),
          const Divider(),
          _buildDrawerItem(Icons.logout_rounded, "Sign Out", _showLogoutDialog, color: Colors.red),
          const SizedBox(height: 24),
        ],
      ),
    );
